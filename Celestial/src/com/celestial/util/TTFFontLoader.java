...
public class TTFFontLoader implements AssetLoader
{
     ...
     *             an exception is thrown if the asset key is not of FontKey
     *             type
     */
    @Override
	...
	if (assetInfo.getKey() instanceof FontKey)
	{
	    ...
	    Texture t = this.prepareTexture(fontData, graphics,
		    fontKey.getColor());
	...
	else
	{
	    throw new IllegalArgumentException(
		    "The given asset key should be of type: "
			    + FontKey.class.getName());
	}
    }
	...
	return gc.createCompatibleImage(1, 1, Transparency.TRANSLUCENT)
		.createGraphics();
    }
	...
	while (i < glyphsAmount)
	{
		...
		bitmapCharacter
			.setWidth(Character.isWhitespace(chars[0]) ? (int) Math
				.ceil(gm.getAdvanceX()) : (int) Math.ceil(gm
				.getBounds2D().getWidth()));
		bitmapCharacter.setXAdvance((int) Math.ceil(gm.getAdvanceX()));
		result.bitmapFont.getCharSet().addCharacter((int) chars[0],
			bitmapCharacter);
		
		result.imageWidth += bitmapCharacter.getWidth()
			+ (int) Math.ceil(Math.abs(gm.getLSB()));// LSB only
								 // affects the
								 // render image
								 // width
		result.imageHeight = (int) Math.ceil(Math.max(
			result.imageHeight, gv.getGlyphVisualBounds(0)
				.getBounds2D().getHeight()));
		result.charList.add(new CharacterData(chars[0], gm.getLSB(),
			bitmapCharacter));
		++i;
	    }
	...
	int ascentBuffer = this.parseBuffer(fontKey.getAboveAscentBuffer(),
		result.imageHeight);
	int descentBuffer = this.parseBuffer(fontKey.getAboveAscentBuffer(),
		result.imageHeight);
	result.imageHeight += ascentBuffer + descentBuffer;
	result.bitmapFont.getCharSet().setBase(
		fontMetrics.getMaxAscent() + ascentBuffer);// we move the base
							   // down by the value
							   // of ascentBuffer
	result.bitmapFont.getCharSet().setLineHeight(
		fontMetrics.getMaxAscent() + fontMetrics.getMaxDescent()
			+ ascentBuffer + descentBuffer);
	result.bitmapFont.getCharSet().setRenderedSize(font.getSize());
	return result;
    }
    
    /**
     * This mehtod parsers the buffer value. The value should be a numeric value
     * that ends in '%', 'px' or nothing. 'px' is a default measure unit.
     * 
     * @param buffer
     *            the buffer value to be parsed
     * @param imageHeight
     *            the height of the image; required if the buffer value is given
     *            in percents
     ...
     */
    protected Texture prepareTexture(FontData fontData, Graphics2D graphics,
	    Color fontColor)
    {
	BufferedImage bufferedImage = graphics.getDeviceConfiguration()
		.createCompatibleImage(fontData.imageWidth,
	...
	// engine redners it up-side down )
	...
	for (CharacterData characterData : fontData.charList)
	{
	    ...
	    xPos += characterData.lsb
		    + characterData.bitmapCharacter.getWidth()
		    + characterData.widthDelta;
	}
    ...
    protected void setPagesForFont(AssetInfo assetInfo, BitmapFont bitmapFont,
	    Texture... textures)
    {
