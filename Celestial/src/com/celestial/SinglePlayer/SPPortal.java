...
public class SPPortal extends CelestialPortal
{
    ...
    public SPPortal(Celestial parent, Node rootNode, Node guiNode, Camera cam,
	    FlyByCamera flyCam, ViewPort viewPort, AssetManager assetManager,
	    InputManager inputManager, AppSettings settings,
	    SimpleApplication app, Timer timer, Gui gui)
    {
	...
	this.cameraControl = new CameraControl(this, this.cam,
		this.inputManager);
	this.cameraControl.init();
	
	Spatial sky = SkyFactory.createSky(this.assetManager,
		"assets/textures/nightsky.jpg", true);
	...
	this.player.getNode().setShadowMode(
	...
	blockHighlightMat = new Material(assetManager,
	...
	blockHighlightMat.getAdditionalRenderState().setBlendMode(
		BlendMode.Alpha);
			...
			if(player.getPlanet() != null)
				...
				Quaternion planetRotationAmount = new Quaternion()
					.fromAngles(p.amountRotation.getX()
						* FastMath.DEG_TO_RAD,
						p.amountRotation.getY()
							* FastMath.DEG_TO_RAD,
						p.amountRotation.getZ()
							* FastMath.DEG_TO_RAD);
				Quaternion starRotationAmount = new Quaternion()
					.fromAngles(p.amountRevolution.getX()
						* FastMath.DEG_TO_RAD,
						p.amountRevolution.getY()
							* FastMath.DEG_TO_RAD,
						p.amountRevolution.getZ()
							* FastMath.DEG_TO_RAD);
				
				Vector3f playerRotatedByStar = starRotationAmount
					.mult(player.getLocation());
				Vector3f playerToPlanet = playerRotatedByStar
					.subtract(p
						.getCurrentPlanetTranslation());
				Vector3f playerRotatedByPlanet = planetRotationAmount
					.mult(playerToPlanet);
				Vector3f finalDifference = (playerRotatedByPlanet
					.add(p.getCurrentPlanetTranslation()))
	...
	if(this.player.getPlanet() != null)
	{
		...
		case NORTH:
		    this.player.setGravity(this.player.getPlanet()
			    .getForwardVector().mult(gravitySpeed));
		    break;
		case SOUTH:
		    this.player.setGravity(this.player.getPlanet()
			    .getForwardVector().mult(-gravitySpeed));
		...
		case EAST:
		    this.player.setGravity(this.player.getPlanet()
			    .getLeftVector().mult(-gravitySpeed));
		    break;
		case WEST:
		    this.player.setGravity(this.player.getPlanet()
			    .getLeftVector().mult(gravitySpeed));
	...
	if(it.hasNext())
	{
		...
		switch(currFace)
		{
		    ...
		    case NORTH:
			item.getCollisionBox().setGravity(
				item.getPlanet().getForwardVector()
					.mult(gravitySpeed));
			break;
		    case SOUTH:
			item.getCollisionBox().setGravity(
	...
	AudioNode audio_ambient = new AudioNode(this.assetManager,
		"assets/sounds/ambient/DST-ArcOfDawn.ogg", false);
	...
	if(values != null)
	{
	    ...
	    {
		this.showHighlight();
		this.blockHighlightGeom.setLocalTranslation(
			blockLocation.getX() * 3 + 1.5f,
			blockLocation.getY() * 3 + 1.5f,
			blockLocation.getZ() * 3 + 1.5f);
		player.getPlanet().getTerrainNode()
			.attachChild(this.blockHighlightGeom);
	    }
	    else
	    {
		this.hideHighlight();
	...
	if(InputControl.statson)
	{
		...
		this.PlanetText.setText("Planet "
			...
			+ "\n\rFace: "
			+ this.player.getCurrentFaceOfPlanet(this.player
				.getPlanet()) + "\n\rInside Atmosphere: "
		...
		for(int x = 0; x < this.player.getPlanet().getDiameter(); x++)
				...
				.getDiameter(); z++)
			{
			    ...
			    {
				blocks = blocks
					+ this.player.getPlanet()
						.getTerrControl().getChunks()
    ...
    {
	this.guiFont = this.getParent().getAssetManager()
	...
	ch.setLocalTranslation(
		// center
		this.settings.getWidth() / 2
			- this.guiFont.getCharSet().getRenderedSize() / 3 * 2,
    ...
    {
	this.guiFont = this.assetManager
		.loadFont("Interface/Fonts/Default.fnt");
	...
	this.InvText
		.setLocalTranslation(10, this.settings.getHeight() - 120, 0);
	...
	this.PlanetText.setLocalTranslation(10, this.settings.getHeight() - 50,
		0);
	this.guiNode.attachChild(this.PlanetText);
	
	this.Vector3IntPosText = new BitmapText(this.guiFont, false);
	this.Vector3IntPosText.setSize(this.guiFont.getCharSet()
		.getRenderedSize());
	this.Vector3IntPosText.setLocalTranslation(10,
		this.settings.getHeight() - 30, 0);
	this.guiNode.attachChild(this.Vector3IntPosText);
	
	this.blocksLiveText = new BitmapText(this.guiFont, false);
	this.blocksLiveText
		.setSize(this.guiFont.getCharSet().getRenderedSize());
	this.blocksLiveText.setLocalTranslation(10,
		this.settings.getHeight() - 150, 0);
    ...
    {
	return new Object[] { this, this.assetManager, this.inputManager,
		this.getParent().getAudioRenderer(), this.getViewPort(),
		this.flyCam };
    }
