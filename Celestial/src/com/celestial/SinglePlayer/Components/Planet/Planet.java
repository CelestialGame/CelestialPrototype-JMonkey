...
public class Planet {
	...
	private void generatePlanet() {
		...
		terrainControl = new BlockTerrainControl(portal.csettings, new Vector3i(diameter, diameter, diameter),
				new NaiveMesher());
			...
			public void onSpatialUpdated(BlockChunkControl blockChunk) {
				...
				if (transparentGeometry.getTriangleCount() > 0) {
					...
					transparentRigidBodyControl
							.setCollisionShape(new MeshCollisionShape(transparentGeometry.getMesh()));
				}
			}
		});
		...
		if (this.type.hasAtmosphere()) {
			this.atmospherebox = new Box(this.diameter * CHUNK_SIZE * 3 * atmosphereSizeFactor,
					this.diameter * CHUNK_SIZE * 3 * atmosphereSizeFactor,
					this.diameter * CHUNK_SIZE * 3 * atmosphereSizeFactor);
			this.atmospheregeom = new Geometry("Atmosphere", this.atmospherebox);
			this.atmospheremat = new Material(portal.getAssetManager(), "Common/MatDefs/Light/Lighting.j3md");

			this.atmospheremat.setBoolean("UseMaterialColors", true);
			this.atmospheremat.setBoolean("UseAlpha", true);
			switch (type) {
			case HABITABLE:
				this.atmospheremat.setColor("Ambient", new ColorRGBA(0.3f, 0.5f, 1, 0.75f));
				this.atmospheremat.setColor("Diffuse", new ColorRGBA(0.3f, 0.5f, 1, 0.75f));
				break;
			case INNER:
				this.atmospheremat.setColor("Ambient", new ColorRGBA(0.68f, 0.4f, 0.09f, 0.75f));
				this.atmospheremat.setColor("Diffuse", new ColorRGBA(0.68f, 0.4f, 0.09f, 0.75f));
				break;
			default:
				this.atmospheremat.setColor("Ambient", new ColorRGBA(0.25f, 0.38f, 0.98f, 0.75f));
				this.atmospheremat.setColor("Diffuse", new ColorRGBA(0.25f, 0.38f, 0.98f, 0.75f));
	...
	private void initLighting() {
		...
		this.directionalLight.setDirection(
				this.getCurrentPlanetTranslation().subtract(this.starNode.getWorldTranslation()).normalize());
		this.starNode.addLight(this.directionalLight);

		this.dlsr = new DirectionalLightShadowRenderer(this.portal.getAssetManager(), 2048, 3);
	...
	protected void drawLODCube() {

	}

	protected void removeLODCube() {
	...
	public Vector3f getSpawnLocation(int face) {
		return new Vector3f(this.planetNode.getWorldTranslation().getX() + 1.5f,
	...
	private void rotate() {
		...
		starNode.rotate(this.amountRevolution.getX() * FastMath.DEG_TO_RAD,
				this.amountRevolution.getY() * FastMath.DEG_TO_RAD, this.amountRevolution.getZ() * FastMath.DEG_TO_RAD);
		planetNode.rotate(this.amountRotation.getX() * FastMath.DEG_TO_RAD,
				this.amountRotation.getY() * FastMath.DEG_TO_RAD, this.amountRotation.getZ() * FastMath.DEG_TO_RAD);
	}
	...
	public Vector3f getUpVector() {
		...
		return new Vector3f(2 * (x * y - w * z), 1 - 2 * (x * x + z * z), 2 * (y * z + w * x));
	}
	...
	public Vector3f getForwardVector() {
		...
		return new Vector3f(2 * (x * z + w * y), 2 * (y * x - w * x), 1 - 2 * (x * x + y * y));
	}
	...
	public Vector3f getLeftVector() {
		...
		return new Vector3f((1 - 2 * (y * y + z * z)), 2 * (x * y + w * z), 2 * (x * z - w * y));
	}
