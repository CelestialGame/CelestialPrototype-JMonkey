...
public class Picker
{
    ...
    private static CollisionResults getRayCastingResults(Node node,
	    CelestialPortal parent, Camera cam)
    {
	Vector3f origin = cam.getWorldCoordinates(new Vector2f((parent
		.getSettings().getWidth() / 2), (parent.getSettings()
		.getHeight() / 2)), 0.0f);
	Vector3f direction = cam.getWorldCoordinates(new Vector2f((parent
		.getSettings().getWidth() / 2), (parent.getSettings()
		.getHeight() / 2)), 0.3f);
    ...
    public static Object[] getCurrentPointedBlock(boolean getNeighborLocation,
	    CelestialPortal parent, Camera cam)
    {
	Object[] values = new Object[3];
	Node terrainNode = parent.galaxy.getPlanet(new SectorCoord(0, 0, 0), 0,
		0).getTerrainNode();
	BlockTerrainControl terrainControl = parent.galaxy.getPlanet(
	...
	if(results.size() > 0)
	{
	    ...
	    Vector3f translatedContactPoint = rotatedContactPoint.subtract(p
		    .getOriginalPlanetTranslation());
	    rotatedContactPoint = q2.mult(translatedContactPoint);
	    
	    rotatedContactPoint = smartRound(rotatedContactPoint);
	    
	    Vector3f relContactPoint = rotatedContactPoint.subtract(p
		    .getOriginalTerrainTranslation().subtract(
			    p.getOriginalPlanetTranslation()));
    ...
    {
	return new Vector3f(StrictMath.round(vec.x), StrictMath.round(vec.y),
	...
	float xd = Float.parseFloat("0."
		+ xs.substring(xs.indexOf(".") + 1, xs.length()));
	float yd = Float.parseFloat("0."
		+ ys.substring(ys.indexOf(".") + 1, ys.length()));
	float zd = Float.parseFloat("0."
		+ zs.substring(zs.indexOf(".") + 1, zs.length()));
