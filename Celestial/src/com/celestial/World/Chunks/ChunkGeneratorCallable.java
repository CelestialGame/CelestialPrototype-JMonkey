...
public class ChunkGeneratorCallable implements Callable<Map<Vector3i, BlockData>> {
	...
	public ChunkGeneratorCallable(Vector3i location, int planetDiameter, int chunkSize, PlanetType planetType,
			long planetSeed) {
	...
	public Map<Vector3i, BlockData> call() throws Exception {
		...
		else if ((x == 1 || x == planetDiameter - 2) && (y == 1 || y == planetDiameter - 2)
		...
		if (this.planetType.hasAtmosphere()) {
			...
			if (x == planetDiameter - 2) {
				...
				generateRandomTerrain(new Vector3i(chunkSize, chunkSize, chunkSize), GameBlock.GRASS,
						PlanetFace.BOTTOM);
				return blockData;
		...
		for (int i = 0; i < chunkSize; i++) {
			for (int j = 0; j < chunkSize; j++) {
				for (int k = 0; k < chunkSize; k++) {
					 ...
					 * if(preChunk.getPlanet().getType
					 * ().equals(planetType.HABITABLE)) makeCubeAt(i,j,k,
					 * BlocksEnum.GRASS, blockTerrain); } else if(k==chunkSize-1
					 * || k==0) {
					 * if(preChunk.getPlanet().getType().equals(planetType
					 * .HABITABLE)) makeCubeAt(i,j,k, BlocksEnum.GRASS,
					 * blockTerrain); } else if(i==chunkSize-1 || i==0) {
					 * if(preChunk
					 * .getPlanet().getType().equals(planetType.HABITABLE))
					 * makeCubeAt(i,j,k, BlocksEnum.GRASS, blockTerrain); } else
					 * {
					 */
					if (x == (planetDiameter - 1) / 2 && y == (planetDiameter - 1) / 2
							&& z == (planetDiameter - 1) / 2) {
						// core
						if (i == chunkSize - 1 || i == 0 || j == chunkSize - 1 || j == 0 || k == chunkSize - 1
								|| k == 0)
							makeCubeAt(i, j, k, GameBlock.SUBSTRATUS);
		...
		/*
		 * //Post-terrain generation here for(int i=0; i<chunkSize; i++) {
		 * for(int j=0; j<chunkSize; j++) { int x1 = (int) (i); int z1 = (int)
		 * (z+j); makeTreeAt(new Vector3Int(x1, 0, z1), blockTerrain); } }
		 */
	...
	private void makeCubeAt(int x, int y, int z, GameBlock blockType) {
		...
		if (blockType.getBClass() == null)
			return;
		this.blockData.put(cubeLocation,
				new BlockData(BlockManager.getInstance().getType(blockType.getBClass()).getType(), cubeLocation, null));
	}
	...
	private void setBlocksFromNoise(Vector3i size, GameBlock blockClass, PlanetFace face) {
		// SimplexNoise simplexNoise=new SimplexNoise(150,0.6,(int)
		// preChunk.getPlanet().getSeed());
		...
		Transform rmft = new Transform();
		rmft.setScale(1.f / 32.f);
		BasicNoise ridgedMF = new BasicNoise(rmft, 0.5f, 0.086f, (int) planetSeed, 2, 3.02f, 1.5f, true, basis);
		Transform simt = new Transform();
		simt.setScale(1.f / 32.f);
